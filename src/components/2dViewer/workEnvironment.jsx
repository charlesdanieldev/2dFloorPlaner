...
// import ObjBoundingBox from "../objBoundingBox";
// import ObjectList from "../objectList";
// import Panel from "../panel";
// import RoomTools from "../roomTools";
// import WallTools from "../wallTools";
import BoxBind from "./boxBind";
import BoxScale from "./boxScale";
import BoxGrid from "./boxGrid";
import BoxPath from "./boxpath";
import BoxRoom from "./boxRoom";
import BoxWall from "./boxwall";
import BoxCarpentry from "./boxcarpentry";
import BoxFurniture from "./boxFurniture";
import BoxArea from "./boxArea";
import BoxRib from "./boxRib";
...
function calcul_snap(event, state, offset) {
  let eX,
    eY,
    x_mouse,
    y_mouse,
  ...
  x_mouse = eX * factor - 1 * factor + originX_viewbox;
  y_mouse = eY * factor - 1 * factor + originY_viewbox;
...
class WorkEnvironment extends Component {
  constructor(props) {
    super(props);
    this.state = { snap: 0, binder: {}, x: 0, y: 0 };
    // this.onMouseMove = this.onMouseDown.bind(this);
  }
  onMouseDown = (event) => {
    console.log(event);
    this.setState({
  ...
  render() {
    let { mode, modeOption, action, grid_snap } = this.props;
    let { x, y } = this.state;
  ...
  //                 );
  //               } else if (
  //                 isObjectsEquals(wall.parent.start, wall.start, "2")
  //               ) {
  //                 newWall = new editor.wall(
  //                   wall.parent.start,
  //                   wall.start,
  ...
  //           ROOM[binder.id].showSurface;
  //         document.querySelector("#roomBackground").value =
  //           ROOM[binder.id].color;
  ...
  //               $("#stepsCounter").show();
  //             }
  //             document.getElementById("bboxWidth").value =
  //               objTarget.width * 100;
  ...
  //   let { mode, modeOption, action, grid_snap } = this.props;
  //   if (mode == "text_mode") {
  //     setSnap(calcul_snap(event, grid_snap));
  ...
  //         if (
  //           editor.rayCastingWalls({ x: binder.bbox.left, y: binder.bbox.top })
  //         )
  //           found = true;
  //         if (
  //           !found &&
  ...
  //             2;
  //         let newAngle = qSVG.vectorDeter(v1, v2);
  //         if (Math.sign(newAngle) == 1) {
  //           angleWall += 180;
  //           binder.x =
  //             pos.x +
  //             (Math.sin(pos.wall.angle * ((360 / 2) * Math.PI)) *
  //               binder.thick) /
  //               2;
  //           binder.y =
  //             pos.y +
  //             (Math.cos(pos.wall.angle * ((360 / 2) * Math.PI)) *
  //               binder.thick) /
  //               2;
  //         }
  ...
  //       });
  //       if (action == 1) {
  //         let startText = qSVG.middle(pox, poy, x, y);
  //         let angleText = qSVG.angle(pox, poy, x, y);
  ...
  //       let pathSurface = roomTarget.coords;
  ...
  //           for (
  //             let free =
  //               Rooms.polygons[roomTarget.inside[ins]].coords.length - 2;
  //             free > -1;
  //             free--
  //           ) {
  //             pathCreate =
  ...
  //           );
  //           let v2 = qSVG.vectorXY({ x: wall.end.x, y: wall.end.y }, snap);
  //           let newAngle = qSVG.vectorDeter(v1, v2);
  //           if (Math.sign(newAngle) == 1) {
  //             angleWall += 180;
  //             binder.angleSign = 1;
  //           }
  //           let startCoords = qSVG.middle(
  //             wall.start.x,
  //             wall.start.y,
  //             wall.end.x,
  //             wall.end.y
  ...
  //           let limits = limitObj(wall.equations.base, binder.size, wallSelect);
  //           if (
  ...
  //         let x2 = addNode.wall.end.x;
  //         let y2 = addNode.wall.end.y;
  //         let x1 = addNode.wall.start.x;
  //         let y1 = addNode.wall.start.y;
  //         angleWall = qSVG.angle(x1, y1, x2, y2);
  //         setBinder(
  //           qSVG.create("boxbind", "path", {
  //             id: "circlebinder",
  //             d:
  //               "M-20,-10 L-13,0 L-20,10 Z M-13,0 L13,0 M13,0 L20,-10 L20,10 Z",
  //             stroke: "#5cba79",
  //             fill: "#5cba79",
  //             "stroke-width": "1.5px",
  //           })
  ...
  //     setSnap(calcul_snap(event, "off"));

  //     let objTarget = false;
  //     for (let i = 0; i < OBJDATA.length; i++) {
  //       let objX1 = OBJDATA[i].bbox.left;
  //       let objX2 = OBJDATA[i].bbox.right;
  //       let objY1 = OBJDATA[i].bbox.top;
  //       let objY2 = OBJDATA[i].bbox.bottom;
  //       let realBboxCoords = OBJDATA[i].realBbox;
  ...
  //           inWallRib(wallList);
  //           let thickObj = wallList.thick;
  //           let sizeObj = objTarget.size;
  ...
  //         let line = qSVG.create("none", "line", {
  ...
  //           stroke: "#5cba79",
  //         });
  //         let ball1 = qSVG.create("none", "circle", {
  //           class: "circle_css",
  //           cx: binder.wall.start.x,
  //           cy: binder.wall.start.y,
  //           r: Rcirclebinder / 1.8,
  //         });
  //         let ball2 = qSVG.create("none", "circle", {
  //           class: "circle_css",
  ...
  //         let fltt = qSVG.angle(pox, poy, x, y);
  //         let flt = Math.abs(fltt.deg);
  //         let coeff = fltt.deg / flt; // -45 -> -1     45 -> 1
  //         let phi = poy - coeff * pox;
  //         let Xdiag = (y - phi) / coeff;
  //         if (typeof binder == "undefined") {
  //           // HELP FOR H LINE
  //           let found = false;
  ...
  //       for (let k in wallListObj) {
  //         let wall = wallListObj[k].wall;
  //         let objTarget = wallListObj[k].obj;
  //         let angleWall = qSVG.angleDeg(
  //           wall.start.x,
  //           wall.start.y,
  //           wall.end.x,
  ...
  //       let intersection1 = qSVG.intersectionOfEquations(
  //         equation1,
  //         equation2,
  //         "obj"
  //       );
  //       let intersection2 = qSVG.intersectionOfEquations(
  //         equation2,
  //         equation3,
  //         "obj"
  //       );
  //       let intersection3 = qSVG.intersectionOfEquations(
  //         equation1,
  //         equation3,
  //         "obj"
  //       );
  ...
  //           let distanceFromStart = qSVG.gap(
  //             equation1.backUp.start,
  //             intersection1
  //           );
  //           let distanceFromEnd = qSVG.gap(equation1.backUp.end, intersection1);
  ...
  //             intersection2
  //           );
  //           let distanceFromEnd = qSVG.gap(equation3.backUp.end, intersection2);
  //           if (distanceFromStart > distanceFromEnd) {
  //             // NEAR FROM End
  ...
  //         let intersectionFollowers = qSVG.intersectionOfEquations(
  ...
  //         let objTarget = equationsObj[rp].obj;
  //         let intersectionObj = qSVG.intersectionOfEquations(
  //           equationsObj[rp].eq,
  //           equation2
  //         );
  ...
  //         let limits = limitObj(equation2, objTarget.size, objTarget);
  //         if (
  ...
  //       for (let k in WALLS) {
  //         let objWall = editor.objFromWall(WALLS[k]); // LIST OBJ ON EDGE
  //         for (let ob in objWall) {
  //           let objTarget = objWall[ob];
  //           let eq = editor.createEquationFromWall(WALLS[k]);
  //           let limits = limitObj(eq, objTarget.size, objTarget);
  //           if (
